...
def select_gene_symbols(
    ...
    removed_by_gene_family_name = asarray(
        tuple(not isna(str_) and any(
            removing_str in str_
            for removing_str in gene_family_name_to_remove)
    ...
    kept_by_locus_type = asarray(
        tuple(not isna(str_) and any(keeping_str in str_
                                     for keeping_str in locus_type_to_keep)
    ...
    for column_name in (
        ...
        plot_table(
            df,
            title='Removed {}'.format(column_name),
        )
